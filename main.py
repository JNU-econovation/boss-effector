    ...
    try:
        async with httpx.AsyncClient(timeout=600.0, follow_redirects=True) as client:
            ...
            response = await client.post(
                f"{GPU_SERVER_URL}/extract-guitar", files=files
            )
        ...
        raise Exception(
            "GPU 서버 타임아웃: 파일이 너무 크거나 서버가 응답하지 않습니다."
        )
    except Exception as e:
        raise Exception(f"기타 추출 실패: {str(e)}")


async def predict_effector_params(
    guitar_sample_path: str, extracted_guitar_path: str
) -> Dict[str, Any]:
    """
    ...
    try:
        async with httpx.AsyncClient(timeout=600.0, follow_redirects=True) as client:
            ...
            files = {
                "guitar_sample": (
                    Path(guitar_sample_path).name,
                    sample_content,
                    "audio/mpeg",
                ),
                "extracted_guitar": (
                    Path(extracted_guitar_path).name,
                    extracted_content,
                    "audio/mpeg",
                ),
            }
            ...
            # GPU 서버 API 호출
            response = await client.post(
                f"{GPU_SERVER_URL}/predict-effector", files=files
            )
...
async def validate_local_file(
    path: Path, filename: str, min_duration: float = 0
) -> tuple:
    """
    ...
    if file_ext not in ALLOWED_AUDIO_FORMATS:
        return (
            False,
            0,
            f"지원하지 않는 파일 형식입니다. {ALLOWED_AUDIO_FORMATS} 만 가능합니다.",
        )
    ...
    try:
        ...
        if duration < min_duration:
            return (
                False,
                duration,
                f"오디오 길이가 너무 짧습니다. 최소 {min_duration}초가 필요합니다. (현재: {duration:.1f}초)",
            )
...
async def process_analysis_stream(
    guitar_sample_path: Path, song_path: Path, guitar_sample_name: str, song_name: str
):
    """
    ...
    try:
        ...
        is_valid, duration, error = await validate_local_file(
            guitar_sample_path,
            guitar_sample_name,
            min_duration=MAX_GUITAR_SAMPLE_DURATION,
        )
        if not is_valid:
        ...
        is_valid, duration, error = await validate_local_file(
            song_path, song_name, min_duration=MIN_SONG_DURATION
        )
        if not is_valid:
        ...
        result = await predict_effector_params(
            str(guitar_sample_path), str(extracted_guitar_path)
        )
    ...
    try:
        ...
        return StreamingResponse(
            process_analysis_stream(
                guitar_sample_path,
                song_path,
                guitar_sample.filename,
                original_song.filename,
            ),
